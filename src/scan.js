/**
 * ! build a clock application in rxjs 
 *  */

import { add } from './helpers';
import { of, Subject, interval,  } from 'rxjs';
import { scan, take } from 'rxjs/operators';

// ! SUM OVER TIME
const data$ = of(1, 2, 3, 4, 5);
data$
    .pipe(
        scan( (acc, value) => acc + value)
    ).subscribe( res => {
        add.li(res);
        console.log("TCL: res", res) // 1, 3, 6, 10, 15
    })


// ! subject : ACCUMULATING AN OBJECT
const dataSubject$ = new Subject();
dataSubject$.pipe(
    scan( (acc, value) => Object.assign({}, acc, value))
).subscribe(val => {
    console.log("TCL: val", val)
})
dataSubject$.next({ name: 'ripon'});
dataSubject$.next({ age: 30}) 
dataSubject$.next({ favoriteLanguage: 'javaScript'});


// Find Fibonacci Number =  xn = n1 + n2
add.li(`fibonacci numbers`)
const fbn$ = interval(1000)
fbn$.pipe(
    take(20),
    scan((acc, value) => {   // initial value here [0,1] and 0
        // ? step 1: set up a number 
        const n = value + 1; 
        // ? step 2: find last value of acc
        const last = acc[n]
        // ? step 3: find last last value of an acc 
        const beforeLast = acc[n-1];

        console.log("TCL: n", n);
        console.log("TCL: last", last);
        console.log("TCL: beforeLast", beforeLast)

        return [...acc, last + beforeLast]

    },[0,1])
).subscribe( res => {
    console.log("TCL: res", res)
    add.li(res)
});



